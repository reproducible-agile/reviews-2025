...
for (i in 1:nrow(submission_table)) {
  ...
  if(!is.na(current$file)) {
    tryCatch({
      ...
      finally = cat("Done with ", current$file, "\n")
    )
  } else {
...
Manual inspection yielded the following details:

- `054`: only has "Software Availability Section"
- some papers were not using the submission template
- `078`: only uses "Data Availability"
- `095`: uses "Availability of data and software"
- `102`: uses "Code and data availability"
...
accepted_papers <- review_data %>%
    dplyr::filter(Decisions == "ACCEPT" | Decisions == "accept?") %>%
    filter(type == "Full paper") %>%
    arrange(id) %>%
    kable() %>%
    kable_styling("striped") %>%
...
1. Familiarise yourself with the [AGILE Reproducibility Review Process](https://docs.google.com/document/d/1JHCQV7GP3YkKwp0Nii3dt3p3Y45hU56Xz2cr-xJVz34/edit#heading=h.oheeg2s92zdm); the following steps are just a tl;dr version
2. Take a look at the [review report template](https://github.com/reproducible-agile/reviews-2025/blob/master/report-template/reproreview-template.Rmd) - even if you're not using it, it gives you guidance
